(() => {
  ...
  if (!registerForm || !countrySelect || !stateSelect || !citySelect) return;
  ...
  const loadCountries = async () => {
    try {
      const response = await fetch(`${apiBaseUrl}/api/catalogs/countries`);
  ...
  const loadStatesByCountry = async (idCountry) => {
    ...
    try {
      const response = await fetch(`${apiBaseUrl}/api/catalogs/countries/${idCountry}/states`);
  ...
  const loadCitiesByState = async (idState) => {
    ...
    try {
      const response = await fetch(`${apiBaseUrl}/api/catalogs/states/${idState}/cities`);
